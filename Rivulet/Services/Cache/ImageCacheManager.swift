...
actor ImageCacheManager: NSObject {
    ...
    /// Get image downsampled to a target pixel size, cached separately from the
    /// full-size variant. Used by the hero backdrop layer during the carousel
    /// entry/paging animation: decoding a 1920px source is roughly 4× cheaper
    /// than the 3840×2160 full-size path, both in memory and CPU.
    ///
    /// After the card settles, `HeroBackdropCoordinator.schedulePendingUpgrade`
    /// upgrades the displayed image to the full-size variant via `imageFullSize`,
    /// so visual fidelity is preserved once motion stops.
    ///
    /// Downsampled and full-size images coexist in the memory cache under
    /// distinct keys (`<hash>@<size>`), so requesting both does not thrash the
    /// cache.
    func imageDownsampled(for url: URL, maxPixelSize: CGFloat) async -> UIImage? {
        let baseKey = cacheKey(for: url)
        let sizedKey = "\(baseKey)@\(Int(maxPixelSize))"

        // 1. Check memory cache at the sized key
        if let cached = memoryCache.object(forKey: sizedKey as NSString) {
            updateAccessTime(for: baseKey)
            return cached
        }

        // 2. Load from disk and downsample off the actor
        if let cacheDir = cacheDirectory {
            let diskImage = await Task.detached(priority: .userInitiated) { [cacheDir] in
                self.loadFromDiskDownsampled(
                    cacheDir: cacheDir,
                    key: baseKey,
            ...
            if let image = diskImage {
                let decoded = await fullyDecodedImageAsync(image)
                memoryCache.setObject(decoded, forKey: sizedKey as NSString)
                updateAccessTime(for: baseKey)
                return decoded
            }
        }

        // 3. Not on disk — download the full asset, then downsample the result.
        // The download path already writes the full bytes to disk, so the next
        // call can take the disk path above without re-hitting the network.
        guard let fullImage = await download(url: url, key: baseKey) else {
            return nil
        }

        // Now re-read from disk at the target size. This is cheap — the file
        // is hot in the OS page cache.
        if let cacheDir = cacheDirectory {
            let diskImage = await Task.detached(priority: .userInitiated) { [cacheDir] in
                self.loadFromDiskDownsampled(
                    cacheDir: cacheDir,
        ...
        // Fallback: if downsample somehow fails, return the full image so the
        // caller still gets something displayable.
        return fullImage
    }
